...
class _HomePageState extends State<HomePage> {
  ...
  void loadTodayData() async {
    DateTime now = new DateTime.now();
    DateTime today = DateTime.utc(now.year, now.month, now.day);
    int todayST = today.millisecondsSinceEpoch;
    int todayED = today.millisecondsSinceEpoch + 3600 * 24 * 1000;
    List<Record> todayRecords = await getRecordsBetween(todayST, todayED);
    todayRecords.forEach((v) => print(v.toMap()));
  }
    ...
    return new Scaffold(
        ...
        bottomNavigationBar: BottomAppBar(
          child: BottomNavigationBar(
            onTap: (index) {
              Navigator.pushNamed(context, ['/', '/summary', '/'][index]);
            },
        ...
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: () {
            // testDB();
            // loadTodayData();
            Navigator.pushNamed(context, '/new-folder');
          },
        ),
        body: ListView(
